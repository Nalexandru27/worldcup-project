...
echo "$($PSQL "SELECT t.name FROM teams t JOIN games g ON t.team_id = g.winner_id WHERE year=2018 AND round='Final' AND t.team_id = g.winner_id")"
...
echo "$($PSQL "SELECT year, t.name FROM teams t JOIN games g ON t.team_id = g.winner_id WHERE round = 'Final' ORDER BY year")"
